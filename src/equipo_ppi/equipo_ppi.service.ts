...
export class EquipoPpiService {
  ...
  constructor(
    ...
    @InjectRepository(EquipoPpiPjic) private readonly repositoryPijc: Repository<EquipoPpiPjic>,
    @InjectRepository(Asignatura) private readonly repositoryAsig: Repository<Asignatura>,
    @InjectRepository(EquipoUsuario) private readonly repositoryEqUsu: Repository<EquipoUsuario>,
    @InjectRepository(SeguimientoPpi) private readonly repositorySeg: Repository<SeguimientoPpi>,
  ...
  async exportData(id: number) {
    ...
    if (id != -1) {
      ...
      hoja.range("A2:F2").merged(true).value([[encabezados.nombre, null, null, null, null, null, null]]);
      hoja.range("A3:D3").merged(true).value([["NOMBRE COMPLETO -  INTEGRANTES", null, null, null]]);
      hoja.range("E3:G3").merged(true).value([["CORREO:", null]]);
      //estudiantes 
      for (let index = 0; index < 3; index++) {
        const n = 4 + index;

        if (estudiantes[index] !== undefined && estudiantes[index] !== null) {
          hoja.range(`A${n}:D${n}`).merged(true).value([[estudiantes[index].usuario.nombre, null, null, null]]);
          hoja.range(`E${n}:G${n}`).merged(true).value([[estudiantes[index].usuario.correo, null, null, null]]);
        } else {
          hoja.range(`A${n}:D${n}`).merged(true).value([["", null, null, null]]);
          hoja.range(`E${n}:G${n}`).merged(true).value([["", null, null, null]]);
        }
      }

      hoja.range("A7:G7").merged(true).value([["DESCRIPCIÓN DETALLADA DEL PROYECTO (Diligencía módulo sol)", null, null, null]]);
      hoja.range("A8:G8").merged(true).value([[encabezados.descripcion, null, null, null]]);
      hoja.range("A9:G9").merged(true).value([["ALCANCE (Diligencia módulo sol)", null, null, null]]);
      hoja.range("A10:E10").merged(true).value([["ALCANCE DETALLADO DEL PROYECTO", null, null, null]]);
      hoja.range("A11:E11").merged(true).value([[encabezados.alcance, null, null, null]]);
      ...
      for (let index = 0; index < citas.length; index++) {
        if (citas[index] !== undefined && citas[index] !== null) {
          ...
          if (seguimiento !== undefined && seguimiento !== null) {
            hoja.range(`A${startRow}:A${endRow}`).merged(true).value([[seguimiento.semana, null, null, null]]);
            hoja.range(`B${startRow}:B${endRow}`).merged(true).value([[format(new Date(citas[index].fecha), 'MMMM d', { locale: es }), null, null, null]]);
            ...
            for (let i = 0; i < 3; i++) {
              ...
              if (usuario !== undefined && usuario !== null) {
                ...
                hoja.cell(`D${startRow + i}`).value(seguimiento[nombre2] === 1 ? 'Si' : 'No');

              }
            }
            hoja.range(`E${startRow}:E${endRow}`).merged(true).value([[seguimiento.observacion, null, null, null]]);
            hoja.range(`F${startRow}:F${endRow}`).merged(true).value([[seguimiento.compromiso, null, null, null]]);
            hoja.range(`G${startRow}:G${endRow}`).merged(true).value([[citas[index].usuariocitaequipo.nombre, null, null, null]]);
          } else {
            hoja.range(`A${startRow}:A${endRow}`).merged(true).value([[null, null, null, null]]);
            hoja.range(`B${startRow}:B${endRow}`).merged(true).value([[null, null, null, null]]);
          }
        } else {
          hoja.range(`A${startRow}:A${endRow}`).merged(true).value([[null, null, null, null]]);
          hoja.range(`B${startRow}:B${endRow}`).merged(true).value([[null, null, null, null]]);
          hoja.range(`E${startRow}:E${endRow}`).merged(true).value([[null, null, null, null]]);
          hoja.range(`F${startRow}:F${endRow}`).merged(true).value([[null, null, null, null]]);
          hoja.range(`G${startRow}:G${endRow}`).merged(true).value([[null, null, null, null]]);
        }
      ...
      hoja.range("A1:G1").style({ fill: { color: 'e2efd9' }, border: true, horizontalAlignment: 'start', bold: true, fontFamily: 'Arial', fontSize: 14 })
      hoja.range("A2:F2").style({ border: true, horizontalAlignment: 'center', bold: false, fontFamily: 'Arial', fontSize: 16 })
      hoja.cell("G2").style({ border: true, horizontalAlignment: 'center', bold: true, fontFamily: 'Arial', fontSize: 16 })
      hoja.range("A3:G3").style({ fill: { color: 'e2efd9' }, border: true, horizontalAlignment: 'center', bold: true, fontFamily: 'Arial', fontSize: 12 })
      hoja.range("A4:G6").style({ border: true, horizontalAlignment: 'center', bold: false, fontFamily: 'Arial', fontSize: 10 })
      hoja.range("A7:G7").style({ fill: { color: 'e2efd9' }, border: true, horizontalAlignment: 'center', bold: true, fontFamily: 'Arial', fontSize: 12 })
      hoja.range("A8:G8").style({ border: true, horizontalAlignment: 'center', bold: false, fontFamily: 'Arial', fontSize: 10 })
      hoja.range("A9:G10").style({ fill: { color: 'e2efd9' }, border: true, horizontalAlignment: 'center', bold: true, fontFamily: 'Arial', fontSize: 12 })
      ...
      for (let index = 0; index < equipos.length; index++) {
        if (equipos[index] !== undefined && equipos[index] !== null) {
          ...
          const hoja = await libro.sheet(equipos[index].codigoEquipo.toString());
          ...
          for (let index = 0; index < citas.length; index++) {
            if (citas[index] !== undefined && citas[index] !== null) {
              const seguimiento = await this.repositorySeg
                .createQueryBuilder('SeguimientoPpi')
                .where('SeguimientoPpi.citas = :idCod', { idCod: citas[index].id })
                .getOne()
              if (seguimiento !== undefined && seguimiento !== null) {
                hoja.range(`A${startRow}:A${endRow}`).merged(true).value([[seguimiento.semana, null, null, null]]);
                hoja.range(`B${startRow}:B${endRow}`).merged(true).value([[format(new Date(citas[index].fecha), 'MMMM d', { locale: es }), null, null, null]]);
                ...
                for (let i = 0; i < 3; i++) {
                  ...
                  const usuario = await this.repositoryUsuario
                    .createQueryBuilder('usuario')
                    .where('usuario.id = :idCod', { idCod: seguimiento[nombre1] })
                    .getOne()
                  if (usuario !== undefined && usuario !== null) {
                    hoja.cell(`C${startRow + i}`).value(usuario.nombre);
                    hoja.cell(`D${startRow + i}`).value(seguimiento[nombre2] === 1 ? 'Si' : 'No');
